...
depths = depths[depths['date'].dt.month.isin(months)]

# Set rows with Month = 9 and Day = 1 to 0
for index, row in depths.iterrows():
    if ((row['date'].to_pydatetime().month == 9) and (row['date'].to_pydatetime().day == 1)):
        depths.loc[index, 'depth_in'] = 0
        
# Any custom clean up
depths.loc[0, 'depth_in'] = 0

# Forward fill over index axis
depths['depth_in'] = depths['depth_in'].ffill(axis = 0) 
